...
def run_weak_key_typing():
    # 생성된 문장 불러오기
    with open("data/generated_sentence.txt", "r", encoding="utf-8") as f:
        sentences = [f.read().strip()]  # 한 줄로 된 문장을 리스트로 변환
    ...
    def on_key_press(event):
        nonlocal last_press_time, start_time
    ...
    #결과 저장
    def save_results():
        nonlocal current_sentence_index, current_sentence, sentence_id, key_logs, start_time, last_press_time
        ...
        # 키 입력 결과 csv 저장
        with open("data/weak_key_typing_results.csv", "w", newline="", encoding="utf-8-sig") as f:
        ...
        with open("data/weak_key_typing_summary.csv", "w", newline="", encoding="utf-8-sig") as f:
            writer = csv.DictWriter(f, fieldnames=summary_data[0].keys())
            writer.writeheader()
            writer.writerows(summary_data)

        print("data/weak_key_typing_results.csv 저장 완료")
        print("data/weak_key_typing_summary.csv 저장 완료")
